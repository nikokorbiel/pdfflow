...
import { Presentation, Sparkles, AlertCircle } from "lucide-react";
import {
  getRemainingUsage,
  getMaxFileSize,
} from "@/lib/usage";
...
export default function PowerPointToPDF() {
  ...
  const convertToPDF = async () => {
    ...
    try {
      ...
      setError("PowerPoint to PDF conversion requires server-side processing. This feature is available with PDFflow Pro. For now, you can use Microsoft PowerPoint's built-in 'Save as PDF' feature, or Google Slides to convert your presentation.");

    } catch (err) {
      console.error("Conversion error:", err);
      setError("Failed to convert presentation. Please try again.");
    } finally {
        ...
        <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="text-center animate-fade-in-up">
            ...
            <p className="mt-4 text-xl text-[var(--muted-foreground)] max-w-lg mx-auto leading-relaxed">
              Convert your presentations to PDF format
            </p>
          </div>
          ...
          <div className="mt-12 space-y-6 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            ...
            <div className="rounded-3xl bg-[var(--muted)]/50 border border-[var(--border)] p-5 animate-fade-in">
              <p className="text-sm text-[var(--muted-foreground)] text-center">
                <strong className="text-foreground">Pro Feature:</strong> PowerPoint to PDF conversion requires server-side processing.
                <br className="hidden sm:block" />
                <span className="text-[var(--accent)]"> Upgrade to Pro</span> for instant presentation to PDF conversion.
              </p>
            </div>
            ...
            <div className="rounded-3xl border bg-[var(--card)] p-6 shadow-glass">
              ...
              <ul className="space-y-3 text-sm text-[var(--muted-foreground)]">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Microsoft PowerPoint:</strong> File → Save As → PDF</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-[var(--accent)]">•</span>
                  <span><strong>Google Slides:</strong> File → Download → PDF document</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-[var(--accent)]">•</span>
                  <span><strong>LibreOffice Impress:</strong> Free open-source alternative with PDF export</span>
                </li>
              </ul>
            </div>
          </div>
