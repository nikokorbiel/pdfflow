...
import { Download, Scissors, Sparkles, AlertCircle, Crown, Archive } from "lucide-react";
import { useToolUsage } from "@/hooks/useToolUsage";
import Link from "next/link";
import JSZip from "jszip";

export default function SplitEveryN() {
  ...
  const splitPDF = async () => {
    ...
    try {
      ...
      const zip = new JSZip();
      const numParts = Math.ceil(pageCount / splitEvery);
      setResultCount(numParts);

      for (let part = 0; part < numParts; part++) {
        setStatus(`Creating part ${part + 1} of ${numParts}...`);
        setProgress(10 + ((part / numParts) * 80));

        const newPdf = await PDFDocument.create();
        const startPage = part * splitEvery;
        const endPage = Math.min(startPage + splitEvery, pageCount);
        ...
        zip.file(`${originalName}_part${part + 1}.pdf`, pdfBytes);
      }
  ...
  const downloadResult = () => {
    ...
    link.href = resultUrl;
    const originalName = files[0]?.name?.replace(/\.pdf$/i, "") || "document";
    link.download = `${originalName}_split.zip`;
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="text-center animate-fade-in-up">
            ...
            <h1 className="mt-6 text-4xl sm:text-5xl font-semibold tracking-tight">Split Every N Pages</h1>
            <p className="mt-4 text-xl text-[var(--muted-foreground)] max-w-lg mx-auto">Split your PDF into multiple files by page count</p>
          </div>
          ...
          <div className="mt-12 space-y-6">
            ...
            {files.length > 0 && !resultUrl && (
              <div className="rounded-3xl border bg-[var(--card)] p-6 shadow-glass">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--muted)] focus:border-rose-500 focus:outline-none"
                />
                <p className="mt-2 text-sm text-[var(--muted-foreground)]">
                  Total pages: {totalPages} | Will create {Math.ceil(totalPages / splitEvery)} file{Math.ceil(totalPages / splitEvery) !== 1 ? "s" : ""}
                </p>
              </div>
            )}
            ...
            {resultUrl && (
              <div className="rounded-3xl border bg-[var(--card)] p-8 shadow-glass text-center">
                ...
                <p className="text-sm text-[var(--muted-foreground)]">Created {resultCount} PDF files in a ZIP archive</p>
              </div>
            )}
