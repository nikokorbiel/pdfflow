...
import { Download, Focus, Sparkles, AlertCircle, Crown } from "lucide-react";
import { useToolUsage } from "@/hooks/useToolUsage";
import Link from "next/link";
import type { PDFDocumentProxy } from "pdfjs-dist";

export default function SharpenPDF() {
  ...
  const applySharpen = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        setStatus(`Sharpening page ${i} of ${totalPages}...`);
        ...
        // Apply sharpening using convolution filter
        ...
        // Sharpen kernel (scaled by amount)
        const amount = sharpenAmount;
        const kernel = [
          0, -amount, 0,
      ...
      setError("Failed to sharpen PDF. Please try again.");
    } finally {
  ...
  const downloadResult = () => {
    ...
    link.download = `${originalName}_sharpened.pdf`;
          ...
          <div className="text-center animate-fade-in-up">
            <div className="relative inline-flex">
              ...
              <div className="relative flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-emerald-500 to-green-500 shadow-lg">
                <Focus className="h-10 w-10 text-white" />
              </div>
            </div>
            <h1 className="mt-6 text-4xl sm:text-5xl font-semibold tracking-tight">Sharpen PDF</h1>
            <p className="mt-4 text-xl text-[var(--muted-foreground)] max-w-lg mx-auto">Enhance edges and details in your PDF</p>
          </div>
          ...
          <div className="mt-12 space-y-6">
              ...
              <div className="rounded-3xl border bg-[var(--card)] p-6 shadow-glass space-y-4">
                <div>
                  <div className="flex justify-between mb-2">
                    <label className="text-sm font-medium text-[var(--muted-foreground)]">Sharpen Amount</label>
                    <span className="text-sm text-[var(--muted-foreground)]">{sharpenAmount.toFixed(1)}</span>
                  </div>
                  <input
                    type="range"
                    min="0.1"
            ...
            {resultUrl && (
              <div className="rounded-3xl border bg-[var(--card)] p-8 shadow-glass text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">PDF Sharpened!</h3>
                <p className="text-sm text-[var(--muted-foreground)]">Sharpening applied with {sharpenAmount.toFixed(1)} strength</p>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              {resultUrl && (
                <button onClick={() => { setFiles([]); setResultUrl(null); setSharpenAmount(1); }} className="flex items-center justify-center gap-2 rounded-full border-2 px-8 py-4 font-medium hover:bg-[var(--muted)] transition-all">
                  Process Another
                </button>
              )}
            </div>
