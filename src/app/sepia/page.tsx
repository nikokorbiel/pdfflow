...
import { Download, Sun, Sparkles, AlertCircle, Crown } from "lucide-react";
import { useToolUsage } from "@/hooks/useToolUsage";
import Link from "next/link";
import type { PDFDocumentProxy } from "pdfjs-dist";

export default function SepiaPDF() {
  ...
  const applySepia = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        setStatus(`Applying sepia to page ${i} of ${totalPages}...`);
      ...
      setError("Failed to apply sepia effect. Please try again.");
    } finally {
  ...
  const downloadResult = () => {
    ...
    link.download = `${originalName}_sepia.pdf`;
        ...
        <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="text-center animate-fade-in-up">
            <div className="relative inline-flex">
              <div className="absolute inset-0 bg-gradient-to-br from-amber-600 to-yellow-600 rounded-3xl blur-xl opacity-40" />
              <div className="relative flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-amber-600 to-yellow-600 shadow-lg">
                <Sun className="h-10 w-10 text-white" />
              </div>
            </div>
            <h1 className="mt-6 text-4xl sm:text-5xl font-semibold tracking-tight">Sepia PDF</h1>
            <p className="mt-4 text-xl text-[var(--muted-foreground)] max-w-lg mx-auto">Give your PDF a vintage sepia tone</p>
          </div>
          ...
          <div className="mt-12 space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[var(--muted-foreground)]">
                      Your PDF will get a warm, vintage sepia effect
                    </p>
                  </div>
                </div>
              </div>
            ...
            {resultUrl && (
              <div className="rounded-3xl border bg-[var(--card)] p-8 shadow-glass text-center">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-600 to-yellow-600 mb-4">
                  <Sun className="h-8 w-8 text-white" />
                </div>
                <h3 className="text-lg font-semibold mb-2">Sepia Applied!</h3>
                <p className="text-sm text-[var(--muted-foreground)]">Your PDF now has a vintage look</p>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              {!resultUrl ? (
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-3 rounded-full bg-gradient-to-r from-amber-600 to-yellow-600 px-8 py-4 font-medium text-white shadow-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
