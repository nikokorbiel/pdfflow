...
import { Download, FilePlus, Sparkles, AlertCircle, Crown } from "lucide-react";
...
export default function AddBlankPages() {
  ...
  const addBlankPages = async () => {
    ...
    try {
      ...
      setStatus("Adding blank pages...");
      setProgress(50);

      // Get the size of the first page to match
      const firstPage = pdfDoc.getPage(0);
      const { width, height } = firstPage.getSize();

      for (let i = 0; i < numPages; i++) {
        pdfDoc.insertPage(
          position === "start" ? i : position === "end" ? pdfDoc.getPageCount() : afterPage + i,
          [width, height]
        );
      }
      ...
      setError("Failed to add blank pages. Please try again.");
    } finally {
  ...
  const downloadResult = () => {
    ...
    link.download = `${originalName}_with_blank_pages.pdf`;
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="text-center animate-fade-in-up">
            ...
            <h1 className="mt-6 text-4xl sm:text-5xl font-semibold tracking-tight">Add Blank Pages</h1>
            <p className="mt-4 text-xl text-[var(--muted-foreground)] max-w-lg mx-auto">Insert blank pages into your PDF</p>
          </div>
          ...
          <div className="mt-12 space-y-6">
            ...
            {files.length > 0 && !resultUrl && (
              <div className="rounded-3xl border bg-[var(--card)] p-6 shadow-glass space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[var(--muted-foreground)] mb-2">Number of blank pages</label>
                    <input
                      type="number"
                      min="1"
                      max="100"
                      ...
                      className="w-full px-4 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--muted)] focus:border-sky-500 focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[var(--muted-foreground)] mb-2">Insert position</label>
                    <select
                      value={position}
                ...
                {position === "after" && totalPages > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-[var(--muted-foreground)] mb-2">Insert after page (1-{totalPages})</label>
                    <input
                      type="number"
                      min="1"
                      max={totalPages}
                      value={afterPage}
                      onChange={(e) => setAfterPage(Math.max(1, Math.min(totalPages, parseInt(e.target.value) || 1)))}
                      className="w-full px-4 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--muted)] focus:border-sky-500 focus:outline-none"
                    />
                  </div>
                )}
            ...
            {resultUrl && (
              <div className="rounded-3xl border bg-[var(--card)] p-8 shadow-glass text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">Pages Added!</h3>
                <p className="text-sm text-[var(--muted-foreground)]">{numPages} blank page{numPages > 1 ? "s" : ""} added to your PDF</p>
              </div>
            )}
